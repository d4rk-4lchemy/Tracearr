...
function buildSessionStarted(ctx: SessionContext): PushoverMessage {
  const { session } = ctx;
...
function buildSessionStopped(ctx: SessionContext): PushoverMessage {
  const { session } = ctx;
  const { title: mediaTitle, subtitle } = getMediaDisplay(session);
  const userName = getUserDisplayName(session);
  const mediaDisplay = subtitle ? `${mediaTitle} - ${subtitle}` : mediaTitle;
...
function buildServerDown(ctx: ServerContext): PushoverMessage {
  return {
...
function buildServerUp(ctx: ServerContext): PushoverMessage {
  return {
    title: 'Server Online',
...
function buildPluginUpdate(ctx: PluginUpdateContext): PushoverMessage {
  return {
